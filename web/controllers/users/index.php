...
$app->match('/users/create', function () use ($app) {
	...
	$form = $form->add('fname', 'text', array('required' => true));
	$form = $form->add('lname', 'text', array('required' => true));
	$form = $form->add('password', 'text', array('required' => true));
	$form = $form->add('email', 'text', array('required' => false));
	$form = $form->add('role', 'text', array('required' => false));
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $update_query = "INSERT INTO `users` (`phone`, `fname`, `lname`, `password`, `email`, `role`, `createdAt`, `updatedAt`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            $app['db']->executeUpdate($update_query, array($data['phone'], $data['fname'], $data['lname'], $data['password'], $data['email'], $data['role'], $data['createdAt'], $data['updatedAt']));            
...
$app->match('/users/edit/{id}', function ($id) use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $update_query = "UPDATE `users` SET `phone` = ?, `fname` = ?, `lname` = ?, `password` = ?, `email` = ?, `role` = ?, `createdAt` = ?, `updatedAt` = ? WHERE `userid` = ?";
            $app['db']->executeUpdate($update_query, array($data['phone'], $data['fname'], $data['lname'], $data['password'], $data['email'], $data['role'], $data['createdAt'], $data['updatedAt'], $id));            
